...
export default async function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const now = new Date()
  const allBookings = seedBookings(now).filter((b) => b.clientId === id)
  ...
  const pkg = seedClientPackages(now).find((p) => p.clientId === id)
  const pkgData = pkg ? getPackage(pkg.packageId) : null
  const clientTriggers = seedTriggers(now).filter((t) => t.clientId === id)
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="pt-0">
          <div className="flex flex-col md:flex-row md:items-end gap-4 -mt-12">
            <Avatar className="h-24 w-24 ring-4 ring-card shadow-lift">
              <AvatarFallback
                className="text-2xl"
